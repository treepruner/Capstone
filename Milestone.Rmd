...
The news file is written in standard, business English from an objective viewpoint. The blog file is more conversational English centered around personal experiences. The twitter file is filled with slang, abbreviations, profanity, emojois that in some cases may only be completely understood by the twitter user's followers. 
...
Specific profanity identified by the 458 "bad words" from Google will be replaced with the literal string 'profanity'.
...
#### News file
readr package read_lines function required for the news file.

```{r loadNews, echo = TRUE}
...
# raw data examples and counts 
news_ex <- as.data.frame( head(news,1,))
...
news_tokens_ex <- head(news_tokens,1)
...
print(news_ex)
print(news_tokens_ex)

# counts
Labels <- c("Source", "Original", "Tokens", "Features")
...
# raw data examples and counts
blogs_ex <-  as.data.frame( head(blogs,1))
...
# capture corpus examples and counts
blogs_tokens_ex <- head(blogs_tokens,1)  
...
# raw data examples and counts
twitter_ex <-  as.data.frame( head(twitter,1))
...
# capture corpus examples and counts
twitter_tokens_ex <- head(twitter_tokens,1)  
...
print(twitter_ex)
print(twitter_tokens_ex)

# counts
Labels <- c("Source", "Original", "Tokens", "Features")
twitter_Counts <- c("twitters", twitter_rowCnt, twitter_tokensCnt, twitter_featureCnt )
